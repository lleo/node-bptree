...
function BpTree(order, keyCmp, storage, opts) {
  ...
  opts.cow = opts.cow ? true : false

  this.opts = u.clone(opts)
 ...
 */
BpTree.prototype.loadRootHandle = function(cb){
  var self = this
  this.storage.loadRootHandle(function(err, hdl){
    if (err) { cb(err); return }

    if (hdl !== null) {
      self.load(hdl, function(err, node){
        if (err) { cb(err); return }
        self.root = node
        cb(null)
      })
...
BpTree.prototype.findLeaf = function(key, cb){
  ...
  function find(err, node) {
    ...
    assert.ok(node instanceof Leaf, format("BpTree#findLeaf: node=%j", node))
  ...
  this.doMerge(leaf, path, function(err, mergedLeaf, pKey, deadLeaf){
    ...
    if ( !mergedLeaf.toBig() ) {
      ...
      self.store(mergedLeaf, function(err, newHdl){
        ...
        self.removeFromBranch(parent, pKey, mergedLeaf, path, deadHdls, cb)
      })
      return
    }
  ...
  this.doMerge(branch, path, function(err, mergedBranch, pKey, deadBranch){
    ...
    if ( !mergedBranch.toBig() ) {
      ...
      self.store(mergedBranch, function(err, hdl){
        ...
        self.removeFromBranch(parent, pKey, mergedBranch, path, deadHdls, cb)
      })
      return
    }
...
//  console.warn("BpTre#replaceRoot: newRoot=%s; oldRoot=%s;", newRoot, oldRoot)
  ...
  self.cleanHandles(deadHdls, function(err){
    if (err) { cb(err); return }
...
//    assert.ok(typeof n.children[i] != 'undefined')
  ...
  this.storage.store(n, this.opts.cow ? null : node.hdl, function(err, hdl){
    self.writing = false
