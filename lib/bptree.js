...
BpTree.prototype.removeFromLeaf = function(leaf, key, opData, cb) {
  ...
  this.mergeNode(leaf, opData, function(err, mergedLeaf, pKey, deadLeaf){
    ...
    if ( !mergedLeaf.toBig() ) {
      ...
      self.store(mergedLeaf, function(err){
        if (err) { cb(err); return }

        opData.stored(mergedLeafOldHdl, mergedLeaf.hdl)
    ...
    self.store(rLeaf, function(err){
      if (err) { cb(err); return }

      opData.stored(rLeafOldHdl, rLeaf.hdl)

      parent.add(npKey, mergedLeaf.hdl, rLeaf.hdl)

      self.storeNode( mergedLeaf, opData, cb)
    }) //.store rLeaf
  }) //.mergeNode
} //.removeFromLeaf
...
BpTree.prototype.removeFromBranch = function(branch, key, rNode, opData, cb){
  ...
  if (this.root === branch) {
    assert.ok( branch.parent === null
             , "BpTree#removedFromBranch: branch.parent !== null" )

    if (branch.size() > 1) {
      this.storeNode(branch, opData, cb)
    }
    else {
      ...
      rNode.parent = null
      this.storeNode(rNode, opData, cb)
    }
  ...
  //branch.toSmall needs .mergeNode

  this.mergeNode(branch, opData, function(err, mergedBranch, pKey, deadBranch){
    ...
    if ( !mergedBranch.toBig() ) {
      ...
      self.store(mergedBranch, function(err){
        if (err) { cb(err); return }

        opData.stored(mergedBranchOldHdl, mergedBranch.hdl)
    ...
    self.store(rBranch, function(err){
      if (err) { cb(err); return }

      opData.stored(rBranchOldHdl, rBranch.hdl)

      parent.add(npKey, mergedBranch.hdl, rBranch.hdl)

      self.storeNode( mergedBranch, opData, cb)
    }) //.store rBranch
  }) //.mergeNode
} //.removeFromBranch
...
BpTree.prototype.storeLeaf = function(leaf, opData, cb){
  assert(leaf instanceof Leaf, "storeLeaf: leaf !instanceof Leaf")
...
BpTree.prototype.storeNode = function(node, opData, cb){
  ...
  this.store(node, function(err){
    ...
    if (parent.dirty || !oldHdl.equals(node.hdl)) {
...
BpTree.prototype.store = function(node, cb){
  ...
  this.storage.store(n, hdl_, function(err, newHdl){
    self.writing = false
