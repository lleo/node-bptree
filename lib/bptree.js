...
BpTree.prototype.del = function(key, cb){
  ...
  this.findLeaf(key, function(err, leaf, path){
    ...
    self.removeFromLeaf(leaf, key, path, function(err, oldHdls, newHdls){
      if (err) { cb(err); return }
...
//              , path.map(function(p){ return p.toString() }) )
  ...
  this.store(node, function(err, newHdl){
    ...
    newHdls.push(newHdl) // node.hdl === newHdl after .store
    oldHdls.push(oldHdl)
    ...
    if ( path.length > 0 ) {
      ...
      if (parent.dirty || oldHdl.cmp(newHdl) != 0) {
      //if (true) {
...
//      console.warn("BpTree#storeSimplePath: replacing root w/node=%s", node)
      self.replaceRoot(node, newHdls, oldHdls, cb)
    }
  })
}
...
BpTree.prototype.storeSplitPath = function(lNode, pKey, rNode, path, newHdls, oldHdls, cb){
  ...
  , function(err, res){
      newHdls.push(lNode.hdl)
      oldHdls.push(lHdl)

      newHdls.push(rNode.hdl)
      oldHdls.push(rHdl)

      if ( path.length == 0 ) {
        var newRoot = new Branch(self.order, [pKey], [lNode.hdl, rNode.hdl], self.keyCmp)
        self.replaceRoot(newRoot, newHdls, oldHdls, cb)
        return
      }
...
//  console.warn("BpTree#removeFromLeaf: removed key=%j; leaf=%s", key, leaf)
  ...
  if (this.root === leaf) {
    ...
    if (leaf.size() > 0) {
      ...
      assert.equal(leaf.keys.length, leaf.children.length)
      this.replaceRoot(leaf, [], [], cb)
    }
    else {
...
//    console.warn("BpTree#removeFromLeaf: did merge: pKey=%s; mergedLeaf=%s; deadLeaf=%s;", pKey, mergedLeaf, deadLeaf)

    var parent = path.pop()
      //??? is this the right way to handle deadLeaf
      , oldHdls = [deadLeaf.hdl]
      , newHdls = [null]

    if ( !mergedLeaf.toBig() ) {

      oldHdls.push(mergedLeaf.hdl)

      self.store(mergedLeaf, function(err, hdl){
        if (err) { cb(err); return }

        newHdls.push(mergedLeaf.hdl)
    ...
    //parent MUST be a branch!
    parent.remove(pKey, mergedLeaf.hdl) //<-before any store

    oldHdls.push(rLeaf.hdl)

    self.store(rLeaf, function(err, hdl){
      if (err) { cb(err); return }

      newHdls.push(rLeaf.hdl)

      parent.add(npKey, rLeaf.hdl)

      path.push(parent)

      self.storeSimplePath( mergedLeaf, path, newHdls, oldHdls, cb)
    }) //.store rLeaf
  }) //.doMerge
} //.removeFromLeaf
 ...
 *
 */
BpTree.prototype.removeFromBranch = function(branch, key, rNode, path, newHdls, oldHdls, cb){
...
//  console.warn("\nBpTree#removedFromBranch: key=%s; branch=%s; rNode=%s;", key, branch, rNode)
  ...
  branch.remove(key, rNode.hdl)

//  console.warn("BpTree#removedFromBranch: after remove: branch=%s;", branch)

  if (this.root === branch) {
    assert.equal(path.length, 0, "BpTree#removedFromBranch: path.length != 0")

    if (branch.size() > 1) {
      //branch root must have at least one key (and two children)
      assert.ok(branch.keys.length > 0)
      assert.ok(branch.keys.length+1 == branch.children.length)
//      this.storeSimplePath(branch, path, newHdls, oldHdls, cb)
      this.replaceRoot(branch, newHdls, oldHdls, cb)
    }
    else {
...
//    console.warn("BpTree#removeFromBranch: did merge: pKey=%s; mergedBranch=%s; deadBranch=%s;", pKey, mergedBranch, deadBranch)

    var parent = path.pop()

    //??? is this the right way to handle deadBranch
    oldHdls.push(deadBranch.hdl)
    newHdls.push(null)

    if ( !mergedBranch.toBig() ) {

//      console.warn("BpTree#removedFromBranch: !mergedBranch.toBig")
...
//    console.warn("BpTree#removedFromBranch: mergedBranch.toBig; doing split")

    var pair = mergedBranch.split()
      , npKey   = pair[0]
      , rBranch = pair[1]

//    console.warn("BpTree#removedFromBranch: before remove: parent=%s; pKey=%j; mergedBranchBranch.hdl=%s", parent, pKey, mergedBranch.hdl)
    parent.remove(pKey, mergedBranch.hdl)  //<-before any store
//    console.warn("BpTree#removedFromBranch: after remove: parent=%s", parent)

    oldHdls.push(rBranch.hdl)

    self.store(rBranch, function(err, hdl){
      if (err) { cb(err); return }

      newHdls.push(rBranch.hdl)

//      console.warn("BpTree#removedFromBranch: before add: parent=%s; npKey=%j; rBranch.hdl=%s", parent, npKey, rBranch.hdl)
      parent.add(npKey, mergedBranch.hdl, rBranch.hdl)
//      console.warn("BpTree#removedFromBranch: after add: parent=%s", parent)

      path.push(parent)

//      console.warn("BpTree#removedFromBranch: calling .storeSimplpath on mergedBranch=%s", mergedBranch)

      self.storeSimplePath( mergedBranch, path, newHdls, oldHdls, cb)
    }) //.store rBranch
  }) //.doMerge
} //.removedFromBranch
...
//  console.warn("BpTree#doMerge: node=%s;", node)
...
//    console.warn("BpTree#doMerge: loaded sibNode: sibNode = %s", sibNode)
    ...
    self.mergeOp(lNode, rNode, function(err){
      ...
      //console.warn("BpTree#doMerge: post merge: pKey=%s; lNode = %s", pKey, lNode)
...
BpTree.prototype.cleanHandles = function(newHdls, oldHdls, cb){
  ...
  //this oldHdls/newHdls does not capture the correct information to
  //determine if a hdl should be released. I'll need to refactor the
  //whole design of oldHdls/newHdls :(
  ...
  , function(oldHdl, ecb){
      ...
      //if (oldHdl != null && newHdl == null) {
      //  self.storage.release(oldHdl, ecb)
      //  return
      //}
      //
      //if (oldHdl != null && newHdls != null && oldHdl.cmp(newHdl) != 0) {
      //  self.storage.release(oldHdl, ecb)
      //  return
      //}

      if (oldHdl != null) {
        if (newHdl == null || oldHdl.cmp(newHdl) != 0) {
          console.warn("BpTree#release: oldHdl=%s;", oldHdl)
          if (newHdl == null)
            console.warn("BpTree#release: newHdl == null")
          else
            if (oldHdl.cmp(newHdl) != 0)
              console.warn("BpTree#release: oldHdl != newHdl; oldHdl=%s; newHdl=%s;", oldHdl, newHdl)
      ...
      if (oldHdl == null && newHdl == null)
          console.error("***BpTree#cleanHandles: WTF! oldHdl == newHdl == null")
...
//  console.warn("BpTree#store: STORING n=%j", n)
