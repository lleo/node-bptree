...
function BpTree(order, store, keyType, valType) {
  ...
  this.root = null
  this.writing = false
//CACHE  this.cache = new Cache()

} //constructor

function isValidPackFuncs( packFuncs ) {
  if ( typeof packFuncs.packLength != 'function' ) return false
  if ( typeof packFuncs.pack       != 'function' ) return false
  if ( typeof packFuncs.unpack     != 'function' ) return false
  if ( packFuncs.packLength.length != 1 ) return false
  if ( packFuncs.pack.length       != 3 ) return false
  if ( packFuncs.unpack.length     != 2 ) return false
  return true
}
...
//CACHE  */
//CACHE function Cache(max){
//CACHE   this.max = u.isUndefined(max) ? 1000 : max
//CACHE   this.nodeQ = new Dequeue()
//CACHE   this.nodes = []
//CACHE }
//CACHE
//CACHE Cache.prototype.size = function(){
//CACHE   return this.nodeQ.length
//CACHE }
//CACHE
//CACHE Cache.prototype.put = function(hdl, node){
//CACHE   this.nodeQ.push(hdl)
//CACHE   this.nodes[hdl] = node
//CACHE
//CACHE   var victimHdl
//CACHE   if (this.size() > this.max) {
//CACHE     victimHdl = this.nodeQ.shift()
//CACHE     delete this.nodes[victimHdl]
//CACHE   }
//CACHE   return this
//CACHE }
//CACHE
//CACHE Cache.prototype.get = function(hdl){
//CACHE   return this.nodes[hdl]
//CACHE }
...
BpTree.prototype.insert = function(node, path_, key, child, cb) {
  ...
  async.series(
    ...
    , function(scb){
        //if (node === self.root)
        if (path.length == 0) {
          self.root = new Branch(self.order, [pKey], [node.hdl, sibl.hdl])
...
BpTree.prototype.removeFromLeaf = function(leaf, path_, key, cb) {
  ...
  for (leafIdx=0; leafIdx < parent.children.length; leafIdx++)
    if ( 0 == leaf.hdl.cmp(parent.children[leafIdx]) ) break
...
BpTree.prototype.removeFromBranch = function(node, path_, hdl, cb) {
  ...
  for (nodeIdx=0; nodeIdx < parent.children.length; nodeIdx++)
    if ( 0 == node.hdl.cmp(parent.children[nodeIdx]) ) break
...
BpTree.prototype.load = function(hdl, cb){
  ...
  if (this.store instanceof TrivialStore) {
    this.store.load(hdl, function(err, node){
      cb(err, node)
    })
  }
...
BpTree.prototype.store = function(node, cb){
  ...
  if (this.store instanceof TrivialStore) {
    this.store.store(node, node.hdl, function(err, hdl){
      self.writing = false
      if (err) { cb(err); return }
      node.hdl = hdl
      cb(null, node.hdl)
