...
BpTree.prototype.store = function(node, cb){
  ...
  if      (node instanceof Leaf  ) {
    ...
    n.type = "Leaf"
    n.children = u.clone(node.children, true /*deep*/)
  }
  else if (node instanceof Branch) {
    assert.equal(node.keys.length+1, node.children.length,
                 format("node.keys.length+1 != node.children.length; n = %j",n))

    var Handle = this.storage.constructor.Handle
    n.type = "Branch"
    n.children = node.children.map(function(c){ return Handle.toJSON(c) })

    assert.equal(n.children.length, node.children.length)
    assert.equal(n.keys.length+1, n.children.length,
                 format("n.keys.length+1 != n.children.length; n = %j",n))

    //assert.equal(n.children.length, node.children.length) //TMP
    //assert.equal(n.keys.length+1, n.children.length) //TMP
  }
