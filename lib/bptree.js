...
bptStats.createStat('tt_put ns', Stats.TimerNS)
bptStats.createStat('tt_store ns', Stats.TimerNS)
bptStats.createStat('tt_load ns', Stats.TimerNS)
bptStats.createStat("tt_put ns ravg", Stats.RunningAverage
                  , { stat: bptStats.get("tt_put ns") })
bptStats.createStat("tt_store ns ravg", Stats.RunningAverage
                  , { stat: bptStats.get("tt_store ns") })
bptStats.createStat("tt_load ns ravg", Stats.RunningAverage
                  , { stat: bptStats.get("tt_load ns") })
bptStats.createHistogram("hog tt_put ns", "tt_put ns", Stats.semiLogNS)
bptStats.createHistogram("hog tt_store ns", "tt_store ns", Stats.semiLogNS)
bptStats.createHistogram("hog tt_load ns", "tt_load ns", Stats.semiLogNS)

bptStats.createStat("sz_store_leaf", Stats.Value, {units: "bytes"})
bptStats.createStat("sz_store_branch", Stats.Value, {units: "bytes"})
bptStats.createHistogram("hog sz_store_leaf", "sz_store_leaf", Stats.bytes)

bptStats.createStat('load_cnt', Stats.Count, {units: "#"})
bptStats.createStat('store_cnt', Stats.Count, {units: "#"})
...
BpTree.prototype.put = function(key, value, cb){
  var self = this
    , doneNS = bptStats.get('tt_put ns').start()
  ...
  if (this.root === null) { //empty tree
    ...
    this.replaceRoot(newRoot, [], function(err){
//    this.storeLeaf(newRoot, [], function(err){
...
//      this.storeSimplePath(rNode, path, deadHdls, cb)
    }
...
BpTree.prototype.load = function(hdl, cb){
  ...
  var loadedNS = bptStats.get('tt_load ns').start
  bptStats.get('load_cnt').inc()
  ...
  this.storage.load(hdl, function(err, n){
    ...
    //FYI
    // u.isPlainObject(undefined) => false
    // u.isPlainObject(null)      => false
    // u.isPlainObject(1)         => false
    // u.isPlainObject("string")  => false
    // u.isPlainObject([])        => false
    // u.isPlainObject(new Foo()) => false
    ...
    if ( n.type == 'Leaf') {
      ...
      node = new Branch(n.order, n.keys, children, self.keyCmp)
    }
...
//    assert.ok(typeof n.children[i] != 'undefined')
  ...
  var storedNS = bptStats.get('tt_store ns').start()
  bptStats.get('store_cnt').inc()
