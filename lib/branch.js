...
//DEL    if ( 0 == oldHdl.cmp(this.children[i]) ) {
//DEL    if ( u.isEqual( oldHdl, this.children[i]) ) {
 ...
 */
Branch.prototype.add = function(key, hdl) {
  ...
  this.children.splice(i+1, 0, hdl)
 ...
 *
 * @param {Key} key
 * //depricated: @returns {[idx, childHdl]}
 * @returns {Handle}
 */
Branch.prototype.remove = function(key, cb) {
  ...
  for (i=0; i<this.keys.length; i++) {
    cmp = this.keyCmp(key, this.keys[i])
    if (cmp === 0) {
      hdl = this.children[i+1]
      this.keys.splice(i, 1)
      this.children.splice(i+1, 1)
      //depricated: return [i, hdl]
      return hdl
    }
  }
 ...
 * @returns {Key}
 */
Branch.prototype.removeByHdl = function(hdl, cb) {
  var i, key
  for (i=1; i<this.children.length; i++) {
//    if ( 0 == hdl.cmp(this.children[i]) ) {
    if ( u.isEqual( hdl, this.children[i]) ) {
      key = this.keys[i-1]
      this.keys.splice(i-1, 1)
      this.children.splice(i, 1)
      //depricated: return [i, key]
      return key
    }
  }
  console.error("***Branch.remove: failed to find hdl = %s", key)
  console.error("***Branch.remove: this = %s", this)
  return undefined
} //.removeByHdl()
