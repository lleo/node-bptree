...
function Leaf(order, keys, data) {
  Node.call(this, order, keys, data)
...
Leaf.prototype.add = function(key,value) {
  var i, len, cmp
  //console.log("Leaf.add: key = %j", key)
  //console.log("Leaf.add: value = %j", value)
  //console.log("Leaf.add: keys: = %j", this.keys)
 ...
 *
 * @param {Key} key
 * @returns {[idx, data]} the data item associated with the key.
 */
...
Leaf.prototype.split = function(){
  ...
  pKey = tKeys[0]
  nLeaf = new Leaf(this.order, tKeys, tChildren) //next sibling leaf
...
Leaf.prototype.toString = function(){
  ...
  return format("Leaf(id=%s, keys=[%s], children=[%s])", this.id, kStr, cStr)
}
